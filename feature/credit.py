...
class Credit(object):
    ...
    def transform(self):
        # 利用額 / 限度額
        # ゼロを含むとノイズが増える。
        self.credit['AMT_BALANCE_PER_LMT'] = self.credit['AMT_BALANCE'].replace(0, np.nan) / self.credit['AMT_CREDIT_LIMIT_ACTUAL']
    ...
    def _aggregate_by_prev(self, df):
        ...
        c_prev = pd.merge(c_prev, self.credit[['SK_ID_PREV', 'SK_ID_CURR']].drop_duplicates(), on='SK_ID_PREV', how='left')

        c_prev['CREDIT_LIMIT_LAST_BY_MIN'] = c_prev['CREDIT_LIMIT_LAST'] / c_prev['CREDIT_LIMIT_MIN']

        agg = features_common.group_by_1(c_prev, 'SK_ID_CURR', 'CREDIT_LIMIT_LAST_BY_MIN', 'mean', 'credit_prev_mean(CREDIT_LIMIT_LAST_BY_MIN)', merge=False)
