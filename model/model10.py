...
# Model9 predictedをX14Y5に
class Model10(model.Model):
    def __init__(self, name=None, add_columns=['PREDICTED_DPD']):
        if name is not None:
            model_name = name
        else:
            model_name = 'model10'
...
if __name__ == "__main__":
    ...
    for x,y in xys:
        column = 'PREDICTED_X{}Y{}'.format(x,y)
        m = Model10(name='model10_x{}y{}'.format(x,y), add_columns=[column])
        m.cv()